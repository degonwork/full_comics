...
class AllNewMangaRepo{
  ...
  Future<List<AllNewMangaModel>> fetchAllNewMangaRepo()async{
    ...
    final response = await http.get(Uri.parse('http://10.0.2.2:3000/commic/new-commic'));
    
    if (response.statusCode == 200) {
      List<dynamic> jsonResponse = jsonDecode(response.body);
   
      final listAllNewCommic = jsonResponse.map((e) => AllNewMangaModel.fromJson(e)).toList();
